...
function computeOppPreset(formation: string, strength: StrengthKey): OpponentPreset | null {
  ...
  const oppMidfielders: string = m.d >= 5 ? "Protect the defenders" : m.cdm && pressing < 55 ? "Stay in position" : pressing >= 65 ? "Go forward" : "Stay in position";
  const oppDefenders: string   = m.d >= 5 ? "Stay behind" : m.wb && delta >= 0 ? "Move forward" : "Stay behind";
  const marking: string        = m.d >= 5 ? "Man-to-Man" : "zonal";
  const offside: boolean       = m.d <= 4 && pressing >= 50;
  const playStyle: string      = m.t === 0 ? "counter" : m.cam ? "passing" : m.wm || m.wg ? "wing" : "passing";
...
const EDGE_URL   = "https://egzquylwclewcgpqnoig.supabase.co/functions/v1/osm-counter-tactics";
const ANON_KEY   = import.meta.env.VITE_SUPABASE_ANON_KEY as string;
const AUTH_HEADERS = { "Content-Type": "application/json", Authorization: `Bearer ${ANON_KEY}`, apikey: ANON_KEY } as const;
...
function SliderField({ id, label, description, value, onChange, disabled = false, highlight = false, stylePreset = null }: {
  ...
  return (
    <div className="slider-group">
      <label htmlFor={id}>
        {label}: <strong style={{ color: highlight ? "var(--osm-gold)" : "var(--osm-cyan)" }}>{value}</strong>
      </label>
      <input type="range" id={id} min={min} max={max} step={step} value={value} onChange={onChange} disabled={disabled} aria-label={label} />
...
function FormationTypeBadge({ formation }: { formation: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", padding: "2px 10px", borderRadius: 20, fontSize: "0.78em", fontWeight: 600, background: colors[m.t], border: `1px solid ${borders[m.t]}`, color: "var(--text-bright)", marginLeft: 10, verticalAlign: "middle" }}>
      {labels[m.t]}
    </span>
  );
}
...
function App() {
  ...
  const handleEmailSubscription = (): void => {
    const el = document.getElementById("subscribeEmail") as HTMLInputElement;
    if (!el?.value) { setError("Please enter your email address."); return; }
    setShowGoldenTicket(true);
  };

  const handleGoldenTicketSubmit = (email: string): void => {
    const el = document.getElementById("subscribeEmail") as HTMLInputElement;
    if (el) el.value = "";
    if (!isSubscribed) {
      localStorage.setItem("osm_subscribed", "1");
  ...
  const handleCopyReferral = () => {
    navigator.clipboard.writeText(`${window.location.origin}?ref=${user?.id}`).then(() => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      setUser(u);
      if (!u && window.matchMedia("(display-mode: standalone)").matches) setShowPWALoginModal(true);
    });
  ...
  const handleGoogleLogin  = async () => { const { error } = await supabase.auth.signInWithOAuth({ provider: "google",  options: { redirectTo: window.location.origin } }); if (error) setError(error.message); };
  const handleDiscordLogin = async () => { const { error } = await supabase.auth.signInWithOAuth({ provider: "discord", options: { redirectTo: window.location.origin } }); if (error) setError(error.message); };
  const handleLogout       = async () => { const { error } = await supabase.auth.signOut(); if (error) setError(error.message); else setUser(null); };
  ...
  // ── Cooldown ────────────────────────────────────────────────────────────────
  useEffect(() => {
    const subbed  = localStorage.getItem("osm_subscribed") === "1";
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (r <= 0) {
        const subbed = localStorage.getItem("osm_subscribed") === "1";
        setAdvUsesLeft(subbed ? 2 : 1); setAdvCooldownEnd(null); setAdvCountdown(""); localStorage.removeItem("osm_adv_cooldown"); return;
      }
      const d = Math.floor(r / 86_400_000), h = Math.floor((r % 86_400_000) / 3_600_000), m = Math.floor((r % 3_600_000) / 60_000), s = Math.floor((r % 60_000) / 1_000);
  ...
  const handleFreeCalc = async (): Promise<void> => {
    ...
    try {
      const res = await fetch(EDGE_URL, { method: "POST", headers: AUTH_HEADERS, body: JSON.stringify({ formation: freeInputs.oppFormQuick, strength: freeInputs.strengthQuick, userId: user?.id, isFreeCalculation: true }) });
    ...
    } catch (err: any) { setError(err?.message || "Service temporarily unavailable."); }
  ...
  const generateCounterStrategy = async (): Promise<void> => {
    ...
    try {
      const res = await fetch(EDGE_URL, {
        ...
        body: JSON.stringify({
          formation: inputs.oppForm, strength: inputs.strengthLevel, oppPlayStyle: inputs.oppPlayStyle || "passing",
          oppMarking: inputs.oppMarking, myPressing: inputs.oppPressing, myStyle: inputs.oppStyle, myTempo: inputs.oppTempo,
  ...
  const applyPreset = useCallback((form: string, strength: StrengthKey): void => {
    ...
    if (!p) return;
    setInputs((prev) => ({ ...prev, oppPressing: p.pressing, oppStyle: p.style, oppTempo: p.tempo, oppForwards: p.oppForwards, oppMidfielders: p.oppMidfielders, oppDefenders: p.oppDefenders, oppMarking: p.marking, oppOffside: p.offside, oppPlayStyle: prev.oppPlayStyle || p.playStyle }));
  ...
  return (
    <>
      ...
      <div className="offline-message" id="offlineMessage">⚠️ You are offline — some features may be unavailable</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: "1200px", margin: "0 auto" }}>
          ...
          {user ? (
            <div style={{ display: "flex", alignItems: "center", gap: 15 }}>
              <span style={{ fontSize: "0.95em", color: "#fff" }}>👤 {user.email}</span>
      ...
      {showBanner && (
        <section id="banner" style={{ position: "relative", maxWidth: "1200px", margin: "10px auto 0", padding: "0 20px" }}>
          <img src="https://z-cdn-media.chatglm.cn/files/99db47b1-d36a-4e40-b49c-47e722efce76.png?auth_key=1868379298-2944d7abcb444f6d9e4be31fa6403e10-0-6b1078c70b29374bd1d019b7300a5069" alt="OSM Banner" style={{ width: "100%", borderRadius: "12px", boxShadow: "0 10px 30px rgba(0,0,0,0.6)", display: "block" }} />
          <button onClick={() => setShowBanner(false)} style={{ position: "absolute", top: "12px", right: "30px", background: "rgba(0,0,0,0.8)", border: "none", color: "#fff", fontSize: "28px", width: "36px", height: "36px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>×</button>
        </section>
      )}
      ...
      <main className="glass">
        ...
        <section id="engineInputs" className="card">
          ...
          <div className="opponent-tactics-section">
            ...
            <div className="slider-grid">
              <SliderField id="oppPressing" label="Opponent Pressing" description="When they trigger the press (low = deep, high = high press)" value={inputs.oppPressing} onChange={handleSlider} stylePreset={inputs.oppPlayStyle ? STYLE_SLIDER_PRESETS[inputs.oppPlayStyle] : null} />
              <SliderField id="oppStyle"    label="Opponent Style"    description="Risk level (low = cautious, high = aggressive)"               value={inputs.oppStyle}    onChange={handleSlider} stylePreset={inputs.oppPlayStyle ? STYLE_SLIDER_PRESETS[inputs.oppPlayStyle] : null} />
              <SliderField id="oppTempo"    label="Opponent Tempo"    description="Play speed (low = slow build-up, high = direct)"              value={inputs.oppTempo}    onChange={handleSlider} stylePreset={inputs.oppPlayStyle ? STYLE_SLIDER_PRESETS[inputs.oppPlayStyle] : null} />
            </div>
        ...
        <div className="subscribe-box">
          <div className="subscribe-box-header">
            ...
            <img src="/images/free-tier-subscribe.png" alt="Subscribe for bonus calculations" className="subscribe-box-img" />
          </div>
          <div className="subscribe-box-body">
            ...
            <div className="subscribe-col">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <button onClick={() => { const el = document.getElementById("referFriendEmail") as HTMLInputElement; if (el?.value) setShowReferModal(true); else setError("Please enter your friend's email address."); }} className="sub-btn-green">Send Referral →</button>
        ...
        {strategy && results && (
          <section id="engineResults" className="card">
            ...
            <div className="slider-grid">
              {([{ key: "recPressing" as const, label: "Your Pressing", desc: "Recommended pressing intensity" }, { key: "recStyle" as const, label: "Your Style", desc: "Recommended risk/attack style" }, { key: "recTempo" as const, label: "Your Tempo", desc: "Recommended play speed" }] as const).map(({ key, label, desc }) => (
            ...
            {(strategy.criticalConstraints?.length ?? 0) > 0 && (
              <div style={{ marginTop: 20 }}>
                ...
                <ul style={{ paddingLeft: 20, display: "flex", flexDirection: "column", gap: 8 }}>
                  {strategy.criticalConstraints!.map((c, i) => <li key={i} style={{ color: c.startsWith("⚠") ? "#ff9800" : "var(--text-bright)", fontSize: "0.9em", lineHeight: 1.55 }}>{c}</li>)}
                </ul>
              </div>
            )}
            {strategies.length > 1 && (
        ...
        {error && (
          <div role="alert" style={{ background: "rgba(255,152,0,.2)", padding: 20, borderRadius: 8, marginTop: 20, border: "1px solid rgba(255,152,0,.5)", color: "#ffa726" }}>❌ {error}</div>
        )}
        ...
        <section id="pricing" className="card">
          ...
          <div className="pricing-grid">
            ...
            <div className="product-card epic-featured">
              ...
              <button onClick={() => setCheckoutTier("epic_lifetime")} style={{ width: "100%", padding: "10px 14px", borderRadius: 8, background: "linear-gradient(135deg,var(--osm-gold),#ffa000)", color: "var(--osm-navy)", border: "none", fontWeight: "bold", cursor: "pointer", fontSize: "0.9em" }}>⭐ Lifetime — €119.95</button>
            ...
            <div className="product-card">
              ...
              <button onClick={() => setCheckoutTier("elite_lifetime")} style={{ width: "100%", padding: "10px 14px", borderRadius: 8, background: "linear-gradient(135deg,var(--osm-gold),#ffa000)", color: "var(--osm-navy)", border: "none", fontWeight: "bold", cursor: "pointer", fontSize: "0.9em" }}>⭐ Lifetime — €299.95</button>
              <p style={{ fontSize: "0.75em", color: "var(--text-dim)", marginTop: 8, textAlign: "center" }}>Lifetime includes all features + auto-updates forever</p>
            </div>
            ...
            <div className="product-card legendary">
              ...
              <div className="pc-img-wrap legendary-image-wrap">
                <img className="product-image legendary-hero-img" src="https://i.ibb.co/YFZBXspw/Gemini-Generated-Image-omupndomupndomup.png" alt="Legendary" />
              ...
              <button onClick={() => setCheckoutTier("legendary_lifetime")} style={{ width: "100%", padding: "10px 14px", borderRadius: 8, background: "linear-gradient(135deg,var(--osm-gold),#ffa000)", color: "var(--osm-navy)", border: "none", fontWeight: "bold", cursor: "pointer", fontSize: "0.9em" }}>⭐ Lifetime — €399.95</button>
              <p style={{ fontSize: "0.75em", color: "var(--text-dim)", marginTop: 8, textAlign: "center" }}>Lifetime includes all features + auto-updates forever</p>
            </div>
      ...
      {new URLSearchParams(window.location.search).get("subscribed") === "true" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(5,15,35,0.98)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "linear-gradient(160deg,#0d1e3a 0%,#091528 100%)", borderRadius: 20, padding: "48px 40px", maxWidth: 520, width: "100%", textAlign: "center", border: "2px solid rgba(0,200,100,.4)", boxShadow: "0 0 60px rgba(0,200,100,.2)" }}>
            ...
            <button onClick={() => { window.history.replaceState({}, "", window.location.pathname); window.location.reload(); }} style={{ padding: "14px 36px", borderRadius: 10, background: "linear-gradient(135deg,#00c864,#00a050)", color: "#fff", border: "none", fontWeight: "bold", fontSize: "1em", cursor: "pointer" }}>Start Using Premium Features →</button>
          </div>
        </div>
      )}
      ...
      {new URLSearchParams(window.location.search).get("checkout") === "cancelled" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(5,15,35,0.98)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "linear-gradient(160deg,#0d1e3a 0%,#091528 100%)", borderRadius: 20, padding: "48px 40px", maxWidth: 520, width: "100%", textAlign: "center", border: "2px solid rgba(255,152,0,.4)", boxShadow: "0 0 60px rgba(255,152,0,.2)" }}>
            ...
            <button onClick={() => { window.history.replaceState({}, "", window.location.pathname); window.location.reload(); }} style={{ padding: "14px 36px", borderRadius: 10, background: "linear-gradient(135deg,var(--osm-gold),#ffa000)", color: "var(--osm-navy)", border: "none", fontWeight: "bold", fontSize: "1em", cursor: "pointer", marginRight: 12 }}>View Plans</button>
            <button onClick={() => { window.history.replaceState({}, "", window.location.pathname); window.location.reload(); }} style={{ padding: "14px 24px", borderRadius: 10, background: "rgba(255,255,255,.1)", color: "var(--text-bright)", border: "1px solid rgba(255,255,255,.2)", fontWeight: "bold", fontSize: "1em", cursor: "pointer" }}>Continue Free</button>
          </div>
        </div>
      )}
      ...
      ══════════════════════════════════════════════════════════════════════ */}
      {showReferModal && (
        <div className="popup-overlay" style={{ display: "flex" }} role="dialog" aria-modal="true" onClick={(e) => { if (e.target === e.currentTarget) setShowReferModal(false); }}>
          <div className="subscription-popup">
            <button className="popup-close" onClick={() => setShowReferModal(false)} aria-label="Close">×</button>
            <div className="popup-header"><h3>👥 Referral Sent!</h3></div>
            <div className="popup-content">
              ...
              <div className="popup-actions">
                <button className="popup-btn primary" onClick={() => { if (user) handleCopyReferral(); const el = document.getElementById("referFriendEmail") as HTMLInputElement; if (el) el.value = ""; setShowReferModal(false); }}>{user ? "🔗 Copy My Referral Link" : "✅ Got It!"}</button>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          INSTALL PWA MODAL  — with image
      ══════════════════════════════════════════════════════════════════════ */}
      {showInstallModal && (
        <div className="popup-overlay" style={{ display: "flex" }} role="dialog" aria-modal="true" onClick={(e) => { if (e.target === e.currentTarget) setShowInstallModal(false); }}>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: "40px" }}>
          ...
          <div>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {[["#freeTier","Free Counter"],["#engineInputs","Advanced Engine"],["#pricing","Pricing"],["/contact","Contact / Support"]].map(([href, label]) => (
            ...
            <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 8 }}>
              <div><span style={{ color: "#ffb400", fontWeight: "bold" }}>🏆 12,847+</span> Matches Won</div>
              <div><span style={{ color: "#ffb400", fontWeight: "bold" }}>⭐ 4.9/5</span> User Rating</div>
              <div><span style={{ color: "#ffb400", fontWeight: "bold" }}>👥 12.8k</span> Active Managers</div>
            </div>
          </div>
          <div>
            ...
            <div style={{ display: "flex", gap: "20px", fontSize: "1.8em" }}>
              {[["🟣","Discord"],["🐦","Twitter"],["▶️","YouTube"],["📘","Facebook"],["🎵","TikTok"]].map(([icon, label]) => (
                <a key={label} href="#" aria-label={label} target="_blank" rel="noopener noreferrer" style={{ color: "#a0b4c8", textDecoration: "none" }} onMouseEnter={(e) => (e.currentTarget.style.color="#00aeef")} onMouseLeave={(e) => (e.currentTarget.style.color="#a0b4c8")}>{icon}</a>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "12px", fontSize: "0.9em" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "15px", paddingTop: "8px", borderTop: "1px dashed rgba(0,174,239,0.1)", fontSize: "0.85em", color: "#6c7a96" }}>
            <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
              <span>🏢 OSM Counter NG Ltd.</span><span>📋 Company No. 12345678</span><span>📍 London, UK</span><span>📞 +44 20 7946 0138</span>
            </div>
